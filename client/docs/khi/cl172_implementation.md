...
**시나리오 B — 코드 전환 X**: 클래스 + Bertha 인스턴스만 생성. 기존 BerthaBossPhaseController / EnemyCatalog 코드 무수정. 실 어댑터 연결은 CL-178.
...
- **expReward / dropWeight 포함**: 시스템 미존재 상태에서 데이터만 선행. CL-178 이후 어댑터/보상 시스템 ticket 에서 연결
...
- **WeaponData Save 패턴 채택 = 본 CL 에 함께 추가**
  ...
  - 현재 Runtime 구독자는 없음 (CL-178 어댑터에서 추가 예상). 이벤트 미구독 상태에서 무해.
...
- `BerthaBossPhaseController.cs` (CL-178 보류)
...
- 명시적 Play 검증은 본 CL 범위 외 (CL-178 어댑터에서 검증).
...
### 위험
1. **데이터-코드 비동기 (의도된 상태)**: Bertha_Boss.asset 의 값은 CL-178 어댑터 작업 전까지 게임에 미반영. 사용자가 SO 만 수정하고 게임 동작이 안 바뀐다고 오해할 수 있음. **팀 공유 필요**.
...
4. **EnemyDataEvents 구독자 없음**: 현재 Runtime 어디에서도 OnAssetSaved 구독하지 않음. ContextMenu Save 는 SaveAssetIfDirty 효과만 발생, Runtime 반영 X. CL-178 어댑터에서 구독자 추가.
...
- [ ] CL-178: BerthaBossPhaseController 가 BossData 를 참조하도록 어댑터 (Configure() API 활용)
- [ ] 일반 EnemyData asset 추가 (일반 몹 등장 ticket)
- [ ] expReward / dropWeight 시스템 연결 (보상/드롭 ticket)
- [ ] EnemyDataPlayModeIsolator 류 Editor 보조 (CL-178 어댑터 후 라이브 튠 필요 시)
