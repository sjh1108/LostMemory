...
namespace LostMemory.Combat
{
    ...
    ///
    /// CL-234: MaxHealthFlat track 추가. 재능 같이 정수 가산 source 가 multiplier 로 들어가
    /// 폭주(예: 10 → x11 → 1100)하던 문제 해결. 합성 공식:
    ...
    public sealed class PlayerHealthStatApplier : MonoBehaviour
    {
        ...
        private void Awake()
        {
            // modifier 적용 전 값을 _baseMaxHealth 로 캡처 — 이후 (base + flat) × mul 로 계산.
            ResolveRefs();
            CaptureBaseIfNeeded();
        }
        ...
        private void CaptureBaseIfNeeded()
        {
            ...
            // 그 경우 health.MaximumHealth = (base + flat) * mul 상태. 역산해서 진짜 base 추출 — 이중 적용 방지.
            ...
            _baseMaxHealth = currentMul > 0f ? (health.MaximumHealth / currentMul) - flatBonus : health.MaximumHealth;
            _lastAppliedMax = health.MaximumHealth;
            _baseCaptured = true;
            if (_logChanges)
                Debug.Log($"[PlayerHealthStatApplier] base MaxHealth captured = {_baseMaxHealth:F0} (flat={flatBonus:F0}, currentMul={currentMul:F2}, observedMax={health.MaximumHealth:F0})", this);
        }
        ...
        private void HandleChanged(StatId stat)
        {
            ...
            if (Mathf.Approximately(newMax, _lastAppliedMax)) return;
            ...
            _lastAppliedMax = newMax;

            if (_logChanges)
                Debug.Log($"[PlayerHealthStatApplier] MaxHealth {oldMax:F0} → {newMax:F0} (flat {flatBonus:F0}, mul {mul:F2}, base {_baseMaxHealth:F0})", this);
        }
    }
}
