...
namespace LostMemory.Dialogue
{
    ...
    /// 시각/입력/타이프라이터 점검 용도.
    /// (V2: develop 의 DialogueTestStarter 와 별도로 공존하는 HEAD 측 버전)
    /// </summary>
    [DisallowMultipleComponent]
    [AddComponentMenu("Lost Memory/Dialogue/Dialogue Test Starter V2")]
    public sealed class DialogueTestStarterV2 : MonoBehaviour
    {
        ...
        private void Fire()
        {
            ...
            if (controller == null)
            {
                Debug.LogError("[DialogueTestStarterV2] DialogueController 가 scene 에 없음. DialoguePanel_v2.prefab 을 Canvas 에 배치하세요.");
                return;
            }
            if (string.IsNullOrEmpty(dialogueGroupId))
            {
                Debug.LogWarning("[DialogueTestStarterV2] dialogueGroupId 가 비어 있음.");
                return;
            }
