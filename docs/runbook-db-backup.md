...
| 항목 | 값 |
|---|---|
| 실행 일시 | 2026-05-16 15:30 KST (`2026-05-16T06:30:19+00:00`, EC2 호스트 TZ=UTC) |
| 실행자 | 송주헌 |
| 백업 파일 | `lostmemory_2026-05-16_063019.sql.gz` |
| 백업 파일 크기 | 5.7K (gzip -9, root:root 644) |
| 소요 시간 | < 1초 |
| `CREATE TABLE` 카운트 | 11 (운영 JPA entity 수와 정합) |
| 디스크 영향 | 24G → 24G (gzip 후 KB 단위라 무시 가능) |
| retention 작동 | OK (첫 실행이라 제거 0건) |
| 결과 | **OK** |
| 비고 | 1차 시도에서 dash 의 `.` source 함정으로 fail → 호스트 .env source 흐름을 제거하고 컨테이너 안 `$POSTGRES_*` 직접 사용으로 fix 후 재실행 성공. 자세한 fix 근거는 commit `bd1a1260e` 참조. |

### 후속 hygiene (이 티켓 밖)

- EC2 호스트 TZ 이 UTC — 운영자 timeline 추적 편의를 위해 호스트 또는 스크립트 출력 timezone 일원화 검토 (별도 티켓)
