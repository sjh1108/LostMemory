...
- 기준 경로를 개인 실험 저장소가 아니라 팀 저장소 `tools/ComfyUI` 기준으로 교체했다.

### 마이너 변경

- `Z-Image-Turbo` 블루프린트와 LoRA 반입 경로 메모를 추가했다.
- 문서 하단에 상세 변경 기록을 추가했다.

## 문서 상태

- 기준 원본: `C:/Users/SSAFY/Downloads/internal_comfyui_gpu_setup_guide_v1.md`
- 반영 기준: 2026-04-21까지 팀이 실제로 검증한 ComfyUI 실행 방식
...
```text
팀 저장소 루트:
C:\Users\SSAFY\Desktop\2학기 3PJT\S14P31C201

ComfyUI 실행 경로:
C:\Users\SSAFY\Desktop\2학기 3PJT\S14P31C201\tools\ComfyUI
```

### 5.2 현재 프로젝트 기준 중요 경로

```text
C:\Users\SSAFY\Desktop\2학기 3PJT\S14P31C201\tools\ComfyUI\models\checkpoints
C:\Users\SSAFY\Desktop\2학기 3PJT\S14P31C201\tools\ComfyUI\models\diffusion_models
C:\Users\SSAFY\Desktop\2학기 3PJT\S14P31C201\tools\ComfyUI\models\loras
C:\Users\SSAFY\Desktop\2학기 3PJT\S14P31C201\tools\ComfyUI\output
```
...
- `Z-Image-Turbo` 예제 블루프린트: `tools/ComfyUI/blueprints`
...
1. 공개 LoRA와 기본 모델 경로 정리
2. `Z-Image-Turbo` baseline 생성과 비교 실험
...
- 개인 실험 저장소 경로를 모두 팀 저장소 `tools/ComfyUI` 경로로 치환했다.
- `checkpoints` 외에 `diffusion_models`, `loras` 경로를 함께 남겨 이후 `Z-Image-Turbo`와 LoRA 반입에 바로 쓸 수 있게 했다.
- 이 문서가 담당하지 않는 범위를 `프로젝트 전용 LoRA 직접 학습`까지 명확히 구분했다.
