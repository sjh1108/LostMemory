...
- 로컬 실행 기준 문서 링크를 새 파일명에 맞게 교체했다.

### 마이너 변경

- pixel art LoRA 실험과 `Z-Image` 전환 계획은 별도 문서에서 관리하도록 연결했다.
- 기존 업데이트 메모를 문서 하단의 상세 변경 기록 형식으로 유지했다.
...
실제 실행 명령과 1회 세팅은 `컴피유아이-로컬-실행-가이드.md`를 기준으로 본다.
LoRA 실험 정리와 `Z-Image` 전환 계획은 `로라-실험-정리와-지이미지-전환-계획.md`에서 따로 관리한다.
...
1. `tools/docs/컴피유아이-로컬-실행-가이드.md` 기준으로 로컬 환경을 준비한다.
...
- 한글 파일명 기준으로 참조 문서 링크를 전부 교체했다.
- `img2img` 실습 문서와 LoRA/Z-Image 운영 문서의 역할을 분리했다.
...
- Windows + NVIDIA 환경에서 `RuntimeError: query is not correctly aligned (strideM)`가 발생할 수 있어 `--use-split-cross-attention` 우회 실행 기준을 `컴피유아이-로컬-실행-가이드.md`에 추가했다.
