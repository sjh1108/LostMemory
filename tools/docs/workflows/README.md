...
| **`LostMemory_Illust_To_LineArt.json`** | 일러스트 → 라인 일러스트 (LoRA 0, denoise 0.55, lineart prompt) — **결과 가장 안정적** | 일러스트 1장 | PNG 3장 (라인 일러스트 / 16×16 / RGBA) |
| **`LostMemory_BustToFullLineArt.json`** | **3단계 chain**: 흉상 → 전신 → 라인 일러스트 → 픽셀 | 흉상 일러스트 1장 | **PNG 4장** (전신 / 라인 / 16×16 / RGBA) |
| **`LostMemory_LineArt_To_Variations.json`** | 선화 일러스트 → 표정/포즈 **5가지 변화** (neutral / smile / surprised / angry / sad) — KSampler 5개 갈래 | 선화 일러스트 1장 | **PNG 5장** (각 표정/포즈) |
